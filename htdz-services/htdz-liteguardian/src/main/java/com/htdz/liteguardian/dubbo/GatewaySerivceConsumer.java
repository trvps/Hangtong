...
@Component
public class GatewaySerivceConsumer implements
	...
	public Boolean isDeviceOnline(boolean local, String deviceName,
			String deviceSession) {
		if (local)
			return gatewaySerivce_Local.isDeviceOnline(deviceName,
					deviceSession);
		else
			return gatewaySerivce_transfer.isDeviceOnline(deviceName,
					deviceSession);
	}

	public RPCResult pushMessageToDevice(boolean local, String deviceName,
			String deviceSession, byte[] data) {
		try {
			if (local)
				return gatewaySerivce_Local.pushMessageToDevice(deviceName,
						deviceSession, data);
			else
				return gatewaySerivce_transfer.pushMessageToDevice(deviceName,
						deviceSession, data);
		} catch (Exception e) {
	...
	public RPCResult closeDevice(boolean local, String deviceName,
			String deviceSession) {
		try {
			if (local)
				return gatewaySerivce_Local.closeDevice(deviceName,
						deviceSession);
			else
				return gatewaySerivce_transfer.closeDevice(deviceName,
						deviceSession);
		} catch (Exception e) {
			LogManager.exception(e.getMessage(), e);
		}
	...
	public RPCResult transferDeviceRequest(RouteInfo ri, String deviceName,
			String deviceSession, byte[] data) {
		try {
			if (ri == null) {
			...
			return gatewaySerivce_transfer.transferDeviceRequest(ri,
					deviceName, deviceSession, data);
		} catch (Exception e) {
			LogManager.exception(e.getMessage(), e);
		}
	...
	public RPCResult transferURLRequest(RouteInfo ri, String serviceName,
		...
		try {
			...
			return gatewaySerivce_transfer.transferURLRequest(ri, serviceName,
					path, headers, params, reqBody);
		} catch (Exception e) {
			LogManager.exception(e.getMessage(), e);
		}
