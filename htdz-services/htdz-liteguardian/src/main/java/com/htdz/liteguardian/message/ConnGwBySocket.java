...
public class ConnGwBySocket {
	...
	@Value("${socket.wait.connection.time}")
	private int waitconnectiontime;
	@Value("${socket.wait.response.time}")
	...
	// 1.连接到网关
	public int Conn2GW(String strIP, int iPort) {
		try {
			// client = new Socket(strIP, iPort);

			client = new Socket();
			SocketAddress remoteAddr = new InetSocketAddress(strIP, iPort);
