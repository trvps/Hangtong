...
@Service
public class CastelMessage_0x2005 extends CastelMessage {
	// 获取消息体对象
	@Override
	public Map<Integer, Object> getMsgBodyMap() {
		...
		if ((msgAnalyze.checkStatus) && (msgBodyLength > 0) && (msgBodyLength == msgAnalyze.msgBody.length)
				&& (msgAnalyze.msgBody != null)) {
			int i = 0;
			while (i < msgAnalyze.msgBody.length) {
				// 获取消息体一条消息KEY
				int keyStartIndex = i;
				short key = msgAnalyze.byte2short(new byte[] { msgAnalyze.msgBody[keyStartIndex].byteValue(),
						msgAnalyze.msgBody[(keyStartIndex + 1)].byteValue() });

				// 获取消息体一条消息长度
				int lengthStartIndex = i + 2;
				short length = msgAnalyze.byte2short(new byte[] { msgAnalyze.msgBody[lengthStartIndex].byteValue(),
						msgAnalyze.msgBody[(lengthStartIndex + 1)].byteValue() });
